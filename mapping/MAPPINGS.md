...
NilKit 3.x treats Minecraft mapping data as **build input**, not as project-owned source data.

## Why mappings are not bundled

Official Mojang/Microsoft mappings are licensed for development use, but their license says the complete, unmodified mappings may not be redistributed. NilKit therefore does not check complete official mapping files into this repository and does not include them in release JARs.

The historical `agaricusb/MinecraftRemapping` repository is attached as an external Git submodule for research and local tooling. Its repository does not currently expose a license file in the GitHub root, so NilKit does not copy, vendor, or relicense its mapping collection.

`tools/MinecraftRemapping` is the single mapping source path. It is a Git submodule and is consumed directly; NilKit does not create or require a `.remapping` staging directory. `.remapping/` remains ignored only so stale local folders from 4.0.0 cannot be committed accidentally.
...
```bash
git clone --recurse-submodules git@github.com:NilKit/NilKit.git
```
...
`mappingToolJar` builds a standalone Java 8-compatible CLI JAR containing only NilKit mapping utility code. No mapping dataset is embedded in that JAR.
